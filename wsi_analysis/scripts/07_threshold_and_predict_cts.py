...
def simple_background_subtraction(image, kernel_size=51):
    """
    Performs simple background subtraction on a grayscale image.
    ...
    kernel_size : int
        Size of the Gaussian kernel used to estimate the background.
        Must be an odd integer. A larger kernel size results in a smoother background estimate.
    
    Returns
    -------
    result : np.ndarray
        The background-subtracted image, normalized to the range [0, 255].
    """
    # Compute the background estimate using Gaussian blur
    background = cv2.GaussianBlur(image, (kernel_size, kernel_size), 0)
    
    # Subtract the background from the original image
    subtracted = cv2.subtract(image, background)
    
    # Normalize the result to the full 8-bit range
...
ds = ds.pp.apply(simple_background_subtraction).pp.threshold(thresholds, channels=markers)
