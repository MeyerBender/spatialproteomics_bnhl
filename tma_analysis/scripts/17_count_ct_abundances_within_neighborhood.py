    ...
    for i, row in metadata.iterrows():
        ...
        for codex_id in matching_codex_ids:
            if codex_id not in sample_dict.keys():
                ...
                continue
            try:
                nhs = [x for x in ['Neighborhood 3', 'Neighborhood 5'] if x in sample_dict[codex_id].pp.get_layer_as_df()['_neighborhoods'].unique()]
                ct_predictions = sample_dict[codex_id].nh[nhs].pp.get_layer_as_df('_la_layers')[f'labels_{level}'].value_counts()
                for ct in list(cts):
